...
class User extends yupe\models\YModel
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            [
                'id, update_time, create_time, middle_name, first_name, last_name, nick_name, email, gender, avatar, status, access_level, visit_time, phone, image',
                'safe',
                'on' => 'search'
            ],
            ...
            // ],
            ['phone', 'match', 'pattern' => '/\+7\([0-9]{3}\)[0-9]{3}\-[0-9]{2}\-[0-9]{2}$/', 'message' => 'Не правильный формат телефона' ],
            ['location, image', 'safe']
        ];
    }
    ...
    public function relations()
    {
        return [
            ...
            'images' => [self::HAS_MANY, 'UserImage', 'user_id'],
        ];
    }
    ...
    public function updateUserPhotos()
    {
        foreach (CUploadedFile::getInstancesByName('UserImage') as $key => $image) {
            $pageImage = new UserImage();
            $pageImage->user_id = $this->id;
            $pageImage->attributes = $_POST['UserImage'][$key];
            $pageImage->addFileInstanceName('UserImage[' . $key . '][image]');
            $pageImage->save();
        }
    }
}
