...
class NewsController extends \yupe\components\controllers\FrontController
{
     ...
     * @throws CHttpException
     */
    public function actionViewPortfolio($slug)
    {
        $model = News::model()->published();
        ...
        $this->render('viewPortfolio', ['model' => $model]);
    }

    /**
     *
     */
    public function actionIndexPortfolio()
    {
        $dbCriteria = new CDbCriteria([
            'condition' => 't.status = :status',
            'params' => [
        ...
        if($category){
            $dbCriteria->compare('t.category_id', $category->id);
        }
        ...
        $this->render('indexPortfolio', ['dataProvider' => $dataProvider, 'category' => $category]);
    }
}
