...
class Product extends yupe\models\YModel implements ICommentable
{
    ...
    public function rules()
    {
        return [
            ...
            [
                'name, title, description, short_description, slug, price, discount_price, discount, data, status, is_special, price_euro',
                'filter',
    ...
    public function attributeLabels()
    {
        return [
            ...
            'price_euro' => Yii::t('StoreModule.store', 'Цена Евро'),
        ];
    }
    ...
    public function getIsProductCart()
    {
        $res = false;
        foreach (Yii::app()->cart->getPositions() as $key => $product) {
    ...
    public function getProductQuanityCart()
    {
        $res = 1;
        foreach (Yii::app()->cart->getPositions() as $key => $product) {
            if($this->id == $product->id) {
