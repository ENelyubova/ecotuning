...
class ProductBackendController extends yupe\components\controllers\BackController
{
    ...
    public function actionCourse()
    {
        ...
        $all = $db
            ->createCommand()
            ->select('id, price_euro')
            ->from('{{store_product}}')
            ->where('price_euro is not null and price_euro <> ""')
            ->queryAll();

        foreach ($all as $product) {
            $price = $product['price_euro'] * $module->course;
            $db
                ->createCommand()
    ...
    {
        $model = new UploadPrice();
        if (isset($_POST['UploadPrice'])) {
            $model->attributes = $_POST['UploadPrice'];
            $model->file = CUploadedFile::getInstance($model, 'file');
            if ($model->validate()) {
                $path = Yii::getPathOfAlias('webroot') . '/uploads/files/' . $model->file->getName();
                $model->file->saveAs($path);
                $model->process($path);
                Yii::app()->user->setFlash('success', 'Прайс загружен');
                $this->redirect(['index']);
            }
        }
        $this->render('upload', [
