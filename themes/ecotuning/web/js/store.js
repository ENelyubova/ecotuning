...
$(document).ready(function () {    
    ...
    */
    ...
    $(document).delegate('.filter-wrapper .filter-wrapper__header', 'click', function(){
        var parent = $(this).parents('.filter-wrapper');
    ...
    $(document).mouseup(function (e){
        if(!$('#store-filter').is(e.target) 
            && $('#store-filter').has(e.target).length === 0) { 
            $('#store-filter').removeClass('active');
            $('body').removeClass('bodymenu');
        }
    });
    $(document).mouseup(function (e){
        if(!$('.filter-wrapper').is(e.target) 
            && $('.filter-wrapper').has(e.target).length === 0) { 
            $('.filter-wrapper').removeClass('active');
        }
    });
    ...
    $('.js-arrow-down').on('click', function () {
        ...
        if(parent.hasClass('active')){
            parent.removeClass('active').find('ul:first').hide();
        } else {
            parent.addClass('active').find('ul:first').show();
        }
        return false;
    });
...
$(document).ready(function () {
    ...
    if($('#order-form').find('input[data-mask="phone"]').is('.data-mask')){
        $('#order-form').find('input[data-mask="phone"]')
            ...
            success: function (data) {
                if (data.result) {
                    ...
                    setTimeout(function(){
                        ...
                        button.removeClass('quick-add-product-to-cart but-svg but-svg-left but-animation').addClass('but-go-cart').attr('href', '/cart').find('span').html("Добавлено");
                    }, 600);
                }
                showNotify(button, data.result ? 'success' : 'danger', data.data);
            }
    ...
    */
    ...
    function getCartTotalCost() {
        ...
        $.each($('.position-sum-price'), function (index, elem) {
            cost += parseFloat($(elem).attr('data-price'));
        });
        var delta = 0;
    ...
    function updateCartTotalCost() {
        cartFullCostElement.html(number_format(getCartTotalCost(), 2, '.', ' ').replace('.00', ''));
        cartFullCostElement2.html(number_format(getCartTotalNoDiscountCost(), 2, '.', ' ').replace('.00', ''));
        cartDiscountCost.html(number_format(getCartDiscountCost(), 2, '.', ' ').replace('.00', ''));
        refreshDeliveryTypes();
        updateShippingCost();
    ...
    function updateFullCostWithShipping() {
        ...
        var fullCost = number_format(costRes.toFixed(2) * 1, 2, '.', ' ');
        cartFullCostWithShippingElement.html(fullCost.replace('.00', ''));
    ...
    function updatePositionSumPrice(tr) {
        ...
        tr.find('.position-sum-price').attr('data-price', sumprice);
        tr.find('.position-sum-price').html(number_format(sumprice, 2, '.', ' ').replace('.00', ''));
        tr.find('.position-full-sum-price').html(number_format(price_old * count, 2, '.', ' ').replace('.00', ''));
        tr.find('.product-price-discount').html(number_format(price_discount * count, 2, '.', ' ').replace('.00', ''));
        updateCartTotalCost();
    }
